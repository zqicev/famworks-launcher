...
export default function OverviewTab({ modpack, busyId, onModpackReload }: Props) {
  ...
  // Корзину-сброс показываем только когда есть что сбрасывать: активный локальный override,
  // либо у локальной сборки зашита кастомная анимация. На стандартной анимации корзины нет.
  const canReset = override != null || (isCustom && !!modpack.character)
