...
const POS_SIGN: Vec3 = [1, 1, 1]   // позиция x,y,z: в skinview3d смещение = напрямую из json (в пикселях рига; значения крошечные)
const EULER_ORDER = 'ZYX'          // порядок применения поворотов (Bedrock/Blockbench)
// root двигает ВСЮ модель. Тот же зеркальный знак, что у конечностей: в .bbmodel root=[0,-30,0],
// экспорт даёт [0,30,0], а показывать надо -30 — значит Y (и Z) инвертируются и здесь.
